...
class GcnMode:
    ...
    def load_cora(self, FLAGS):  # load cora dataset
        ...
        val_mask, test_mask, samplingGlobalAdjWithReduceNode, sampling_idx_range = load_data1(FLAGS.dataset, 0.7, 20,
                                                                                              True, 'client5.csv')
    ...
    def evaluate(self, model, features, support, labels, mask, placeholders, epoch):  # model evaluate
        ...
        if epoch == 400:
            ...
            dataframe.to_csv('scatter5AvgResult.csv', index=False, sep=',')
            fig = plt.figure()
            plt.scatter(x_tsne[:, 0], x_tsne[:, 1], c=preLabel, label="t-SNE")
            fig.savefig('scatter5Avg.png')
            plt.show()
...
class secaggclient:
    ...
    def start(self):  # init parameters and build model
        ...
        # dataframe.to_csv('5.csv', index=False, sep=',')  #
    ...
    def trainModel(self):
        ...
        if self.epoch != self.trainEpoch:  # train the model
            ...
            dataframe.to_csv('client5AvgTrainResult.csv', index=False, sep=',')
    ...
    def preProcessAdj(self, aggeratedAdj):
        ...
        for i in range(size[0]):
            for j in range(size[1]):
                if np.round(aggeratedAdj[i][j]) < 0.000001:
                    aggeratedAdj[i][j] = 0
