...
class GcnMode:
    ...
    def load_cora(self, FLAGS):  # load cora dataset
        ...
        val_mask, test_mask, samplingGlobalAdjWithReduceNode, sampling_idx_range = load_data1(FLAGS.dataset, 0.4, 20,
                                                                                              True, 'client1.csv')
    ...
    def evaluate(self, model, features, support, labels, mask, placeholders, epoch):  # model evaluate
        ...
        if epoch == 400:
            ...
            dataframe.to_csv('scatter1AvgResult.csv', index=False, sep=',')
            fig = plt.figure()
            plt.scatter(x_tsne[:, 0], x_tsne[:, 1], c=preLabel, label="t-SNE")
            fig.savefig('scatter1Avg.png')
            plt.show()
...
class secaggclient:
    def __init__(self, serverhost, serverport, mixData, poxyData):
        ...
        self.delta1 = 0.00001
        self.sensitivity1 = 4
        self.essAndFssStartComputationTime = 0
        self.essAndFssEndComputationTime = 0
        self.ef0ssSendStartTime = 0
        self.ef0ssSendEndTime = 0
        self.clientZss0ComputationStartTime = 0
        self.clientZss0ComputationEndTime = 0
        self.sendZssStartTime = 0
        self.sendZssEndTime = 0
    ...
    def trainModel(self):
        ...
        if self.epoch != self.trainEpoch:  # train the model
            ...
            dataframe.to_csv('client1AvgTrainResult.csv', index=False, sep=',')
    ...
    def register_handles(self):
        ...
        def on_get_ef0(*args):
            ...
            if self.ef0Count == 1:
                ...
                self.clientZss0ComputationStartTime = time.clock()
                self.zss0 = np.array(self.f0) * np.array(self.ass0) + np.array(self.e0) * np.array(self.bss0) + np.array(self.css0)
                self.clientZss0ComputationEndTime = time.clock()

                print('client zss0 computation time:', self.clientZss0ComputationEndTime - self.clientZss0ComputationStartTime)

                self.count = 0
                self.pos = 0

                self.sendZssStartTime = time.clock()
        ...
        def on_get_ss0(*args):
            ...
            if self.count0 == 1:
                ...
                self.essAndFssStartComputationTime = time.clock()
                ...
                self.essAndFssEndComputationTime = time.clock()
                print("ess and Fss Computation Time:", self.essAndFssEndComputationTime - self.essAndFssStartComputationTime)
                ...
                self.ef0ssSendStartTime = time.clock()
        ...
        def on_get_zssSum(*args):
            ...
            self.sendZssEndTime = time.clock()
            print('zss send time:', self.sendZssEndTime - self.sendZssStartTime)
            msg = args[0]
        ...
        def on_agg_parameters(*args):
            ...
            self.gcn.sess.run(assignWeight0)  # assign aggerated weights for each client
            self.gcn.sess.run(assignWeight1)

            # feed_dict = construct_feed_dict(self.features, self.support, self.y_train, self.train_mask, self.placeholders)
            # feed_dict.update({self.placeholders['dropout']: self.FLAGS.dropout})

            out0 = self.model.activations[1]  # the output of first layer
            out1 = self.model.activations[2]  # the output of second layer
